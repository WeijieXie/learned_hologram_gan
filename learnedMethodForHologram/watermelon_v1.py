...
from .neural_network_components import (
    UNet_imgDepth2AP_deprecated_v1,
    UNet_imgDepth2AP_heavyweight_deprecated_v1,
    ResNet_POH,
)
...
class watermelon_v1(nn.Module):
    def __init__(
        ...
        if heavyweight_UNet:
            self.part1 = UNet_imgDepth2AP_heavyweight_deprecated_v1(output_channels=6).to(self.device)
        else:
    ...
    def train_model(self, train_iter, test_iter, num_epochs, lr):
        ...
        for epoch in range(num_epochs):
            ...
            for img_depth in train_iter:
                ...
                l = self.loss(
                    y_hat, img_depth[:, :3]
                )  # compare with the ground truth rgb image
